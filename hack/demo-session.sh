...
say "bin/rabi watch --all   # live fleet view"
( bin/rabi watch --all & WATCH_PID=$!; sleep 18; kill $WATCH_PID 2>/dev/null ) || true
wait "$SEED_PID" 2>/dev/null || true
sleep 1

say "bin/rabi list --tenant demo | head -12"
bin/rabi list --tenant demo | head -12
sleep 2

job_id="$(bin/rabi list --tenant demo -o json | python3 -c '
import sys, json
jobs = json.load(sys.stdin)["jobs"]
done = [j for j in jobs if j["status"]["phase"] == "SUCCEEDED"]
print(done[0]["jobId"] if done else jobs[0]["jobId"])')"

say "bin/rabi get $job_id   # placement audit: why did it land there?"
bin/rabi get "$job_id" -o json | python3 -c '
import sys, json
...
run bin/rabi usage --tenant demo
