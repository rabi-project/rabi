...
#
# Stage 2 — QUANTUM (delegated to Rabi). Submits the QuantumJob via rabi, waits
...
# the device, tracks calibration drift, and records usage. Requires rabi and jq
...
job_id=$(rabi submit -f "$WORK/job.yaml" | cut -f1)
echo "quantum: submitted job $job_id to $RABI_SERVER"

elapsed=0
while :; do
  phase=$(rabi get "$job_id" -o json | jq -r '.status.phase')
  case "$phase" in
    SUCCEEDED) break ;;
...
rabi get "$job_id" -o json | jq '.status.tasks[0].result.data.counts' > "$WORK/result.json"
